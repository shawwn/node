...
bool ExecuteString(v8::Isolate* isolate, v8::Handle<v8::String> source,
                   v8::Handle<v8::Value> name, bool print_result,
                   bool report_exceptions, bool call_result) {
  v8::HandleScope handle_scope(isolate);
...
#if 0
  v8::String::Utf8Value str(isolate, source);
  if (call_result) str += "()";
  const char* cstr = ToCString(str);
  Handle<String> source2 =
      isolate->factory()->NewStringFromUtf8(cstr).ToHandleChecked();
#endif
...
bool Bootstrapper::CompileExtraBuiltin(Isolate* isolate, int index) {
  ...
  return ExecuteString(
    ...
    *reinterpret_cast<v8::Handle<v8::String>*>(&source_code),
    *reinterpret_cast<v8::Handle<v8::String>*>(&script_name),
    false, true, false);
#else
